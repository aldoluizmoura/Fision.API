...
public class CaixaController : BaseController
{
    ...
    public CaixaController(INotificador notificador,
    ...
    {
        var entidades = await _caixaRepository.ObterCaixas();
        var caixas = _mapper.Map<IEnumerable<CaixaViewModel>>(entidades);
    ...
    public async Task<CaixaViewModel> MostrarPorId(Guid id)
    {
        ...
        var caixas = _mapper.Map<CaixaViewModel>(entidade);
    ...
    {
        var entidade = await _caixaRepository.ObterPorId(id);
        return _mapper.Map<Caixa>(entidade);
    }
}
